...
} from "@/lib/api/queries/school-ops-mutations";
...
export const PATCH = apiHandler(
  async (request: Request, context: RouteContext) => {
    ...
    if ("role" in body) {
      ...
      return jsonSuccess(
        await updateSchoolMemberRoleForViewer(
          slug,
          membershipId,
          user,
          body.role
    ...
    if ("volleyballPosition" in body) {
      ...
      return jsonSuccess(
        await updateSchoolMemberVolleyballPositionForViewer(
          slug,
          membershipId,
          user,
          volleyballPosition
    ...
    if ("jerseyNumber" in body) {
      ...
      return jsonSuccess(
        await updateSchoolMemberJerseyForViewer(
          slug,
          membershipId,
          user,
          jerseyNumber
    ...
    throw badRequest(
      "Provide role, volleyballPosition, or jerseyNumber to update."
    );
  }
);
...
export const DELETE = apiHandler(
  async (request: Request, context: RouteContext) => {
    ...
    const { slug, membershipId } = await context.params;
    return jsonSuccess(
      await removeSchoolMemberForViewer(slug, membershipId, user)
    );
  }
);
