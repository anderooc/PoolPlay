...
export const DELETE = apiHandler(async (request: Request, context: RouteContext) => {
  ...
  return jsonSuccess(
    await removeTournamentHostDivision(slug, user, divisionId)
  );
});
...
export const PATCH = apiHandler(async (request: Request, context: RouteContext) => {
  ...
  const body = (await request.json().catch(() => null)) as Record<
    ...
    unknown
  > | null;
  if (!body || !Array.isArray(body.courtIds)) {
    throw badRequest("Provide courtIds.");
  }
  if (!body.courtIds.every((value) => typeof value === "string")) {
    throw badRequest("courtIds must be strings.");
  }
  return jsonSuccess(
    await setTournamentHostDivisionCourts(
      slug,
      user,
      divisionId,
      body.courtIds
