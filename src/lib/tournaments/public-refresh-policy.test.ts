...
describe("registrationAvailabilityOpen", () => {
  it("uses the public availability deadline contract at exact equality", async () => {
    const loaded = await import("./public-refresh-policy");
    const registrationAvailabilityOpen = (
      loaded as unknown as {
        registrationAvailabilityOpen?: (
          status: string,
          availability: { deadline: string | null },
          now: string
        ) => boolean;
      }
    ).registrationAvailabilityOpen;
    assert.equal(typeof registrationAvailabilityOpen, "function");
    assert.equal(
      registrationAvailabilityOpen!(
        "registration_open",
