...
async function authorizeSchedule(tournamentId: string, user: AppUser) {
  ...
  if (!(await canScheduleMatches(tournament, user))) {
    return { error: "Scheduling is not available in the current tournament stage." as const };
  }
...
export async function loadPoolDivisionMatches(
  tournamentId: string,
  divisionId: string
): Promise<
...
export async function loadHostScheduleFillMatches(
  tournamentId: string,
  scope: ScheduleTimesScope
) {
  return scope.type === "division-pools"
    ? loadPoolDivisionMatches(tournamentId, scope.divisionId)
    : loadBracketMatches(tournamentId, scope.bracketId);
}

export async function applyHostScheduleFillForTournament(input: {
  tournamentId: string;
  user: AppUser;
  scope: ScheduleTimesScope;
  ...
  const auth = await authorizeSchedule(input.tournamentId, input.user);
  if ("error" in auth) return { error: auth.error };

  const firstStart = new Date(input.firstStartIso);
  ...
  const occupancy = await loadTournamentCourtOccupancy(input.tournamentId);
  ...
  const rows = proposeMatchTimeFill({
    ...
    firstStart,
    intervalMinutes: clampMatchIntervalMinutes(input.intervalMinutes),
